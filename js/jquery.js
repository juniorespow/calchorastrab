 ...
 */
(function($) {
    var w = ($.browser.msie ? 'paste' : 'input') + ".mask";
    var x = (window.orientation != undefined);
    ...
    $.fn.extend({
        ...
        mask: function(m, n) {
            if (!m && this.length > 0) { var o = $(this[0]); var q = o.data("tests"); return $.map(o.data("buffer"), function(c, i) { return q[i] ? c : null }).join('') }
            n = $.extend({ placeholder: "_", completed: null }, n);
            var r = $.mask.definitions;
            var q = [];
            var s = m.length;
            var u = null;
            var v = m.length;
            $.each(m.split(""), function(i, c) {
                if (c == '?') {
                    v--;
                    s = i
                } else { q.push(r[c] ? new RegExp(r[c]) : null); if (q[q.length - 1] && u == null) u = q.length - 1 }
            });
            return this.each(function() {
                var f = $(this);
                var g = $.map(m.split(""), function(c, i) { if (c != '?') return r[c] ? n.placeholder : c });
                var h = false;
                var l = f.val();
                f.data("buffer", g).data("tests", q);

                function seekNext(a) { while (++a < v) { if (q[a]) return a } return v };

                function shiftL(a) {
                    while (!q[a] && a >= 0) a--;
                    for (var i = a; i < v; i++) { if (q[i]) { g[i] = n.placeholder; var j = seekNext(i); if (j < v && q[i].test(g[j])) { g[i] = g[j] } else break } }
                    writeBuffer();
                    f.caret(Math.max(u, a))
                };

                function shiftR(a) {
                    for (var i = a, c = n.placeholder; i < v; i++) {
                        if (q[i]) {
                            var j = seekNext(i);
                            var t = g[i];
                            g[i] = c;
                            if (j < v && q[j].test(t)) c = t;
                            else break
                        }
                    }
                };
                ...
                function keydownEvent(e) {
                    var a = $(this).caret();
                    var k = e.keyCode;
                    h = (k < 16 || (k > 16 && k < 32) || (k > 32 && k < 41));
                    if ((a.begin - a.end) != 0 && (!h || k == 8 || k == 46)) clearBuffer(a.begin, a.end);
                    if (k == 8 || k == 46 || (x && k == 127)) { shiftL(a.begin + (k == 46 ? 0 : -1)); return false } else if (k == 27) {
                        clearBuffer(0, v);
                        writeBuffer();
                        $(this).caret(u);
                        return false
                ...
                function keypressEvent(e) {
                    if (h) { h = false; return (e.keyCode == 8) ? false : null }
                    e = e || window.event;
                    var k = e.charCode || e.keyCode || e.which;
                    var a = $(this).caret();
                    if (e.ctrlKey || e.altKey) { return true } else if ((k >= 41 && k <= 122) || k == 32 || k > 186) {
                        ...
                        if (p < v) {
                            ...
                            if (q[p].test(c)) {
                                ...
                                var b = seekNext(p);
                                $(this).caret(b);
                                if (n.completed && b == v) n.completed.call(f)
                            }
                        }
                    }
                    return false
                ...
                function clearBuffer(a, b) { for (var i = a; i < b && i < v; i++) { if (q[i]) g[i] = n.placeholder } };

                function writeBuffer() { return f.val(g.join('')).val() };

                function checkVal(a) {
                    var b = f.val();
                    var d = -1;
                    for (var i = 0, pos = 0; i < v; i++) {
                        if (q[i]) {
                            g[i] = n.placeholder;
                            while (pos++ < b.length) {
                                var c = b.charAt(pos - 1);
                                if (q[i].test(c)) {
                                    g[i] = c;
                                    d = i;
                                    break
                    ...
                    if (!a && d + 1 < s) {
                        f.val("");
                        clearBuffer(0, v)
                    } else if (a || d + 1 >= s) { writeBuffer(); if (!a) f.val(f.val().substring(0, d + 1)) }
                    return (s ? i : u)
                };
                f.one("unmask", function() { f.unbind(".mask").removeData("buffer").removeData("tests") }).bind("focus.mask", function() {
                    l = f.val();
                    var a = checkVal();
                    writeBuffer();
                    setTimeout(function() { f.caret(a) }, 0)
                }).bind("blur.mask", function() { checkVal(); if (f.val() != l) f.change() }).bind("keydown.mask", keydownEvent).bind("keypress.mask", keypressEvent).bind(w, function() { setTimeout(function() { f.caret(checkVal(true)) }, 0) });
                checkVal()
            })
        }
    })
